...
const InlineScript = {
  ...
  loadFromUrl(url, forceFetch = false) {
    return __awaiter(this, void 0, void 0, function* () {
      let res;
      if (InlineScript.srcCache[url] === undefined || forceFetch) res = yield (yield fetch(url)).text();
  ...
  filterScripts(elements) {
    ...
    for (const element of elements) if (InlineScript.isValidScriptTag(element)) scriptElements.push(element);
...
class InlineScriptInstance {
  ...
  setRenderFunction(element) {
    ...
    element.render = function (calledAutomatically = false) {
      ...
      (_a = element.inlineScriptAttributes) === null || _a === void 0
        ? void 0
        : _a.forEach(({ name, value }) => {
            try {
