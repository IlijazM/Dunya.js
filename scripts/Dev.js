...
class Dev extends DunyaWrapper_js_1.default {
    ...
    this.args = {
      ...
      plugins: [
        ...
        path.join(
          __dirname,
          'default-plugins',
          'default-inline-script-compiler'
        ),
      ],
      watcherConfig: {},
      props: {},
  ...
  async eventHandlerValidator(event, filePath) {
    ...
    let fileContent;
    if (event !== 'unlink') {
      if (await (await fs.lstat(absolutePath)).isDirectory()) return; // If it is a dir
      fileContent = await fs.readFile(path.join(absolutePath));
