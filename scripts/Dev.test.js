...
function dirTree(filename) {
    var stats = fs.lstatSync(filename), info = path.basename(filename);
...
describe('Dev', () => {
    ...
    test('Watcher', async () => {
        try {
            const dev = new Dev_js_1.default({ ...defaultConfig });
            await dev.init();
            expect(1).toEqual(1);
        }
        catch (e) {
    ...
    test('Template file', async () => {
        ...
        try {
            ...
            await sleep(10);
            expect((await fs.readFile(path.join(outPath, 'test', 'index.html'))).toString()).toEqual(template);
        }
        catch (e) {
            expect(() => {
                throw e;
    ...
    test('Inline script file', async () => {
        ...
        try {
            ...
            expect((await fs.readFile(path.join(outPath, 'Test', 'Test.html'))).toString()).toEqual(`<h1>Hello</h1>
