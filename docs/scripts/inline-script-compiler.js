...
function inlineScript(args) {
    ...
    if (args === undefined) {
        ...
        setTimeout(() => {
            const scriptElement = document.createElement('script')
            scriptElement.innerHTML = compiledScript
...
function addElement(element) {
    compiledScript += 'el = document.querySelector(\'.' + UIDPrefix + element.uid + '\')\n'
    compiledScript += 'el.inlineScript = `' + element.inlineScript + '`\n'
    compiledScript += 'el.render = function() {\n'
    compiledScript += '    try {\n'
    compiledScript += '        handleRenderResults(this, eval(this.inlineScript))\n'
    compiledScript += '    } catch (err) {\n'
    compiledScript += '        handleExceptionResult(this, err)\n'
    compiledScript += '    }\n'
    compiledScript += '    renderAttributes(this)\n'
    compiledScript += '    return this\n'
    compiledScript += '}\n\n'
}
//#endregion